...
func Authenticate(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "auth Job published to queue", "code": 200})
}
...
func SignUp(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "error in hashing": err.Error()})
	}
	...
	if err == nil {
		c.JSON(http.StatusOK, gin.H{"message": " user already exists in database", "code": 307})
		return
	}
	Repo.DB.SignUp(u)
